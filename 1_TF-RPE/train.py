...
def run_epoch(data, model, loss_compute, epoch, opt, fgm):
    """
    迭代一次数据集
    :param data:
    :param model:
    ...
    for i, batch in enumerate(data):
        ...
        fgm.attack()
        out_ = model(batch.src, batch.trg, batch.src_mask, batch.trg_mask)
        loss_ = loss_compute(out_, batch.trg_y, batch.ntokens)
        loss_.backward()
        fgm.restore()
...
def train(data, model, criterion, optimizer):
    ...
    fgm = FGM(model, emb_name='MultiHeaderAttention', epsilon=1.0)
    for epoch in range(EPOCHS): # EPOCES在代码开头指定
        """
        每次迭代一次就在验证集上验证loss
        """
        model.train()
        run_epoch(data.train_data, model, SimpleLossCompute(model.generator, criterion, optimizer), epoch, optimizer, fgm)
        model.eval()

        # 在dev集上进行loss评估
        print('>>>>> Evaluate')
        dev_loss = run_epoch(data.dev_data, model, SimpleLossCompute(model.generator, criterion, None), epoch, optimizer, fgm)
