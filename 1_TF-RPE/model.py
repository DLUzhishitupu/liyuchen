...
def make_model(src_vocab, tgt_vocab, N=LAYERS, d_model=D_MODEL, d_ff=D_FF, h=H_NUM, dropout=DROPOUT):
    ...
    model = Transformer(
        ...
        nn.Sequential(AdaptiveEmbedding(src_vocab, d_model, d_model, [d_model / 2], div_val=1.0).to(DEVICE),
                      c(position)),
        nn.Sequential(AdaptiveEmbedding(tgt_vocab, d_model, d_model, [d_model / 2], div_val=1.0).to(DEVICE),
                      c(position)),
